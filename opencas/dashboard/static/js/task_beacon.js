(function (root, factory) {
  ...
  function _renderTopItem(bucket, escapeHtml) {
    const item = bucket.item;
    if (!item) {
      return '<span class="task-beacon-top-item muted">No matching fragments.</span>';
    }
    let html = '<span class="task-beacon-top-item">';
    html += `<span class="task-beacon-top-item-title">${_escape(_bucketItemLabel(item), escapeHtml)}</span>`;
    const meta = _bucketItemMeta(item, escapeHtml);
    if (meta) {
      html += `<span class="task-beacon-bucket-item-meta muted">${meta}</span>`;
    }
    html += '</span>';
    return html;
  }
  ...
  function _renderBucketSummary(bucket, escapeHtml) {
    ...
    html += _renderTopItem(bucket, escapeHtml);
    html += '</span>';
    html += '</summary>';
    return html;
  }
