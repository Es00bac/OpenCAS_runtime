...
if TYPE_CHECKING:
    ...
    from opencas.governance.web_trust import WebTrustService
...
class SelfApprovalLadder:
    ...
    def evaluate(self, request: ActionRequest) -> ApprovalDecision:
        ...
        if web_assessment is not None:
            if web_assessment.blocked:
                score = 1.0
            else:
                score += web_assessment.risk_delta
                reasons.append(f"web_mod={web_assessment.risk_delta:+.3f}")
            reasons.extend(web_assessment.reasons)
    ...
    def _web_trust_modulation(self, request: ActionRequest):
        ...
        if self.web_trust is None:
            return None
        payload = request.payload or {}
        action_class = payload.get("web_action_class")
        if not action_class:
            return None
        return self.web_trust.assess(
