...
def _phone_integration_module():
    return importlib.import_module("opencas.phone_integration")
...
class PhoneBridgeService:
    ...
    async def autoconfigure_twilio(
        ...
        *,
        ...
        integration_module = _phone_integration_module()
        ...
        webhook_secret = str(webhook_secret or self.config.webhook_secret or integration_module.secrets.token_urlsafe(24)).strip()
        ...
        async with integration_module.httpx.AsyncClient(auth=(credentials.api_username, credentials.api_password), timeout=30.0) as client:
    ...
    def validate_webhook_signature(
        ...
        *,
        ...
        validator = _phone_integration_module().RequestValidator(auth_token)
        params: Any
    ...
    def _discover_account_sid(self, api_username: str, api_password: str) -> Optional[str]:
        try:
            response = _phone_integration_module().httpx.get(
    ...
    def validate_owner_pin(self, candidate: str) -> bool:
        ...
        if not configured:
            return True
        return _phone_integration_module().secrets.compare_digest(configured, str(candidate or "").strip())
    ...
    def _start_owner_reply_task(
        ...
        *,
        ...
        token = integration_module.secrets.token_urlsafe(12)
        ...
        *,
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=integration_module._PHONE_OWNER_STREAM_REPLY_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
        ...
        *,
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=integration_module._PHONE_WORKSPACE_STREAM_REPLY_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
            pass
        except Exception:
    ...
    def validate_webhook_request(
        ...
        *,
        ...
        if self.config.webhook_secret:
            if not bridge_token or not integration_module.secrets.compare_digest(
                str(bridge_token),
                str(self.config.webhook_secret),
            ):
                self._trace_signature_failure(
