...
class PluginStore:
    ...
    async def _ensure_schema(self) -> None:
        ...
        cursor = await self._db.execute("PRAGMA table_info(plugins)")
        columns = {row[1] for row in await cursor.fetchall()}
        if "bundle_metadata" not in columns:
            await self._db.execute("ALTER TABLE plugins ADD COLUMN bundle_metadata TEXT")
    ...
    async def install(
        ...
        await self._db.execute(
            """
            INSERT INTO plugins (
                plugin_id, name, description, source, path, enabled, installed_at, updated_at, manifest, bundle_metadata
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            (
                ...
                json.dumps(bundle_metadata) if bundle_metadata is not None else "{}",
            ),
        )
        await self._db.commit()
    ...
    async def list_installed(self) -> List[Dict[str, Any]]:
        ...
        async with self._db.execute(
            """
            SELECT plugin_id, name, description, source, path, enabled, installed_at, updated_at, manifest, bundle_metadata
            FROM plugins
            ORDER BY plugin_id
            """
        ) as cursor:
    ...
    def _row_to_dict(row: aiosqlite.Row) -> Dict[str, Any]:
        ...
        bundle_metadata_raw = row[9] or "{}"
        bundle_metadata = json.loads(bundle_metadata_raw)
        return {
            "plugin_id": row[0],
            "name": row[1],
            "description": row[2],
