 ...
 * state tracking, and persistence. Implements the OpenCAS agent's desire to
...
export interface NotificationConfig {
  enabled: boolean;
  minIntensityThreshold: number; // Legacy UI hint; not a hard notification gate
  quietHoursStart: number; // Legacy field retained for config compatibility
  quietHoursEnd: number; // Legacy field retained for config compatibility
  cooldownMinutes: number; // Legacy field retained for config compatibility
  maxNotificationsPerDay: number; // Legacy field retained for config compatibility
...
export interface NotificationState {
  ...
  currentFocus: string | null; // What the OpenCAS agent is currently "thinking about"
...
export const DEFAULT_NOTIFICATION_CONFIG: NotificationConfig = {
  ...
  quietHoursEnd: 0,
  cooldownMinutes: 0,
  maxNotificationsPerDay: Number.MAX_SAFE_INTEGER,
};
...
export class DaydreamService {
  ...
  public shouldNotifyForSpark(spark: DaydreamSpark): { shouldNotify: boolean; reason: string } {
    ...
    void spark;
    return { shouldNotify: true, reason: 'available_for_agent_judgment' };
  }
