...
- The sibling `open_llm_auth` workspace available at `/mnt/xtra/open_llm_auth/`, because `requirements.txt` installs it as an editable dependency
...
The current repo depends on the editable sibling checkout in `requirements.txt`. Make sure `/mnt/xtra/open_llm_auth/` exists, or change that dependency path before installing.
